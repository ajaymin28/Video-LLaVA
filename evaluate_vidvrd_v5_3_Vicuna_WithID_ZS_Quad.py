...
if __name__=="__main__":
    ...
    splits = ["test"]
    imagenet_vidvrd_root = "/root/jbhoi/gits/vrdone/datasets/vidvrd/videos"
    # imagenet_vidvrd_video_path = os.path.join(imagenet_vidvrd_root, "videos")
    imagenet_vidvrd_annot_root = "/root/jbhoi/gits/vrdone/datasets/vidvrd/annotations"
    dataset = VidVRD(imagenet_vidvrd_annot_root, imagenet_vidvrd_root, splits)

    inference_output_dir  = f"/root/jbhoi/gits/VRDFormer_VRD/data/vidvrd/inference_outputs_onevision/{args.output_dir}" 
    inference_prog_output_dir  = f"/root/jbhoi/gits/VRDFormer_VRD/data/vidvrd/inference_outputs_onevision/{args.output_dir}/prog" 
    ...
    test_data_dir = os.path.join(imagenet_vidvrd_annot_root, "test")
    ...
    for val_id_idx, val_id in enumerate(val_ids):
        ...
        video_path = os.path.join(imagenet_vidvrd_root, video_id+".mp4")
        ...
        for frame_block_index, overall_annot in enumerate(overall_annotations):
            ...
            outputs_unclean = get_model_output(file=file,batch_of_frames=Block_frame_ids, prompt=defaultPrompt)
        ...
        try:
            ...
            print(f"error saving file: {inference_prog_output_dir}/{val_id_idx}_{len(val_ids)}.txt")
